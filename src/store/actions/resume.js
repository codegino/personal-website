...
export const fetchResume = () => async dispatch => {
  ...
  const skillsRef = new Promise(res => {
    databaseRef.child('resume').child('skills').on('value', snapshot => {
  ...
  const experiencesRef = new Promise(res => {
    databaseRef.child('resume').child('experiences').on('value', snapshot => {
  ...
  const certificationsRef = new Promise(res => {
    databaseRef.child('resume').child('certifications').on('value', snapshot => {
  ...
  const educationRef = new Promise(res => {
    databaseRef.child('resume').child('education').on('value', snapshot => {
